...
void addLiteral(char* literal) {
    ...
    literalTable[literalCount].address = -1; // Address not assigned yet
    literalCount++;
}
...
int main() {
    ...
    printf("Enter assembly-like instructions (enter a blank line to end):\n");
    
    // Read lines from standard input until a blank line is entered
    while (size < MAX_INPUT_LINES) {
        fgets(line, sizeof(line), stdin);
        if (line[0] == '\n') {
            break; // Stop reading input on blank line
        }
        
        // Allocate memory for the input line and store it
        input[size] = (char *)malloc(strlen(line) + 1);
